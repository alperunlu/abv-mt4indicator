...
//---- input parameters
extern int ExtABVAppliedPrice=0;
//---- buffers
double ExtABVBuffer[];
//+------------------------------------------------------------------+
...
//---- 
   for(i=nLimit; i>=0; i--)
     {
      if(i==Bars-1)
         ExtABVBuffer[i]=Volume[i];
      else
      
         double dCurrentPrice=GetAppliedPrice(ExtABVAppliedPrice, i);
         double dPreviousPrice=GetAppliedPrice(ExtABVAppliedPrice, i+1);
         
         if(dCurrentPrice==dPreviousPrice)  
         {
            ExtABVBuffer[i]=ExtABVBuffer[i+1];
         }
      ...
      if (hacim == 1 && fiyat == 1)
      {
      ExtABVBuffer[i]=ExtABVBuffer[i+1]+Volume[i];  
      }          
